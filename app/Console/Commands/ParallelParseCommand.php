...
class ParallelParseCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:parallel-parse-command';
    ...
    public function handle()
    {
        ...
        $base_path = base_path();
        echo 3333;
        // Artisan::call('queue:work --max-jobs=8');
        while (count($processes) < $maxProcesses) {
            $process = Process::start("php $base_path\artisan queue:work");
            // $process = new ProcessProcess(['php', 'artisan', 'queue:work']);
            // $process->start();
            $processes[] = $process;
        }
        // echo "321";
        ...
        if ($handle) {
            ...
            while (($line = fgets($handle)) !== false) {
                ...
                if (count($logs) == 1200) {
                    ...
                    dispatch(new SqliteHandleJob($ipsToInsert, $ips, $logs));
